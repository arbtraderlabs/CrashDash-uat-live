...
// Current instrument lifecycle state, kept deliberately separate from the
// historical signal (ADR-034). The signal below is a preserved historical
// fact; this badge reports what is true about the instrument NOW. A non-active
// instrument must never make the historical signal look invalid, deleted or
// wrong -- the wording says the signal is preserved and explains what changed.
const INSTRUMENT_LIFECYCLE = {
  DELISTED: ["DELISTED", "Historical signal preserved. Instrument is no longer active."],
  NO_RECENT_PRICES: ["NO RECENT PRICE DATA", "Historical signal preserved. Current market data is no longer available."],
  DATA_STALE: ["DATA STALE", "Historical signal preserved. Current price feed is stale."],
  INACTIVE: ["INACTIVE", "Historical signal preserved. Instrument is no longer active."],
};

function instrumentLifecycle(model) {
  const status = String(model?.current_instrument_status || "").trim().toUpperCase();
  const entry = INSTRUMENT_LIFECYCLE[status];
  // ACTIVE (or unknown) instruments render exactly as before.
  if (!entry) return null;
  const [label, note] = entry;
  const lastPrice = model?.last_price_date ? ` Last price ${escapeHtml(humanDate(model.last_price_date) || model.last_price_date)}.` : "";
  return { status, label, note: `${note}${lastPrice}` };
}

function renderInstrumentLifecycleBadge(model) {
  const lifecycle = instrumentLifecycle(model);
  if (!lifecycle) return "";
  return `<div class="instrument-lifecycle"><span class="lifecycle-badge lifecycle-${escapeHtml(lifecycle.status.toLowerCase())}">${escapeHtml(lifecycle.label)}</span><p class="lifecycle-note">${lifecycle.note}</p></div>`;
}

function renderDeterministicRiskPanel(model) {
  const severity = String(model.watch_severity || "").trim().toUpperCase();
  const lifecycle = instrumentLifecycle(model);
  const lifecycleNotice = renderInstrumentLifecycleBadge(model);
  if (!severity || severity === "UNAVAILABLE") {
    // The risk panel would normally be omitted, but a preserved historical
    // signal on a non-active instrument still has to say so.
    return lifecycleNotice ? `<aside class="deterministic-risk-panel is-muted" aria-label="CrashDash historical signal">${lifecycleNotice}</aside>` : "";
  }
  const signal = String(model.signal_state || "").trim();
  const muted = lifecycle ? " is-muted" : "";
  return `<aside class="deterministic-risk-panel severity-${severityClass(severity)}${muted}" aria-label="CrashDash deterministic risk">
