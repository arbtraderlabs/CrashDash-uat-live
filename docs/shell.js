...
const learnMenu = document.querySelector("[data-learn-menu]");
document.querySelector("[data-learn-resources]").innerHTML = renderLearnResources();
...
function updateNav() {
  if (["learn", "guide", "expect"].includes(state.view)) learnMenu.open = true;
...
function render() {
  updateNav();
  if (state.view === "guide") { root.innerHTML = renderHowToUseCrashDash(); return; }
  if (state.view === "expect") { root.innerHTML = renderWhatToExpectCrashDash(); return; }
